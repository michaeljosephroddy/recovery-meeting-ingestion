...
- [x] (2026-05-24T12:55+01:00) Reduced AA `scrape_low_confidence` warnings from 219 to 103. Added tab-separated schedule extraction for El Paso, German credential/contact markers for Germany online rows, meeting-page context carryover for TSML detail pages, and safer inline name parsing for rows with labels such as `Type:`, `Meeting-ID`, `Passwort`, `PIN`, `Kenncode`, `Einwahl-Nr.`, and `Code:`. Persisted clean focused reruns for El Paso, Metrolina, Germany, and a regenerated full Salinas artifact because the current live Salinas crawl only returns one page.
- [x] (2026-05-24T13:25+01:00) Ran the next low-confidence cleanup batch concurrently. Dry-ran eight top sources in parallel, persisted clean/full results for Akron, Pueblo, and Santa Clara, and imported a regenerated full Lancaster artifact after its live crawl returned one fewer candidate than the active source count. AA `scrape_low_confidence` warnings fell from 103 to 75 with zero open AA errors. A second concurrent dry-run batch found no safe clean imports because sources either reproduced their warnings, returned incomplete crawls, or failed.
- [x] (2026-05-24T13:55+01:00) Reduced AA `scrape_low_confidence` warnings from 75 to 51. Added extraction guards for non-meeting `Meeting Information` detail rows, Wordfence blocks, source announcement text, colon-style event headings, and online schedule rows that put the real meeting name before `Connect with Zoom`. Regenerated and imported clean San Mateo and Midcoast artifacts, then persisted a clean live Vancouver rerun with 669 normalized candidates and 0 flags.
...
- Observation: Some source reruns need artifact re-extraction rather than a live scrape.
    Evidence: Salinas saved HTML re-extracted to 72 candidates with 0 flags after the parser fixes, but the current live source crawl visited 2 pages and extracted only 1 record. Persisting the live crawl would have incorrectly marked existing Salinas meetings missing, so a regenerated artifact summary from the saved pages was dry-run and then imported instead.
- Observation: German online meeting rows use localized credential markers and can have meeting names that include the word `Zoom`.
    Evidence: Germany warnings contained rows such as `Telefonmeeting ... Pin`, `Zoom Meetings ID ... Passwort`, `Zoom-Meetings-ID ... Kenncode`, and `Einwahl-Nr. ... Code:`. The parser now treats those as listed meeting access data while avoiding a split on bare leading `Zoom` in names like `Zoom-Beginner-Meeting`.
- Observation: Concurrent dry-runs are useful for finding safe wins, but write decisions still need per-source count checks.
    Evidence: The parallel batch found clean dry-runs for Akron, Pueblo, Santa Clara, and Lancaster. Lancaster's live summary returned 52 candidates against 53 active meetings, so it was regenerated from saved pages before import. San Mateo's patched live crawl returned 92 candidates against 126 active meetings, so it was not persisted even though review flags were zero.
- Observation: Some low-confidence rows are not meetings at all.
    Evidence: San Mateo detail pages rendered `Meeting Information Sunday, 7:00 am...` as an inline schedule row, Vancouver's homepage rendered `BAY GROUP: MONDAY 8PM SPEAKER MEETING` from upcoming events, London returned Wordfence timestamp text, and Cortland contained source announcements about future meeting changes. These should be filtered, not normalized as meetings.
- Observation: Midcoast online rows needed a second time/range cleanup before name recovery.
    Evidence: Rows such as `7:00am - 8:00am Zoom Meeting Morning Medicine ... Connect with Zoom...` were using the first time as the meeting time, then leaving the second time and `Zoom Meeting` prefix in the name candidate. Cleaning the leading range fragment and splitting at `Connect with Zoom` recovered names while preserving meeting IDs, passwords, and dial-in numbers.
...
- Decision: Leave damaged low-confidence rows open instead of resolving the queue to zero.
    ...
    Date/Author: 2026-05-24 / Codex.
- Decision: Regenerate artifact summaries from saved HTML when the live source now exposes fewer pages than the prior successful scrape.
    Rationale: The importer marks meetings missing based on the candidate set it receives. For Salinas, the current live crawl produced a clean but incomplete one-record set, while the saved pages contained the full schedule and re-extracted cleanly with the updated parser. Re-extracting saved HTML preserved the complete source view and avoided a false source drop.
    Date/Author: 2026-05-24 / Codex.
- Decision: Keep concurrent warning cleanup to dry-runs and inspection, then persist successful sources one at a time.
    Rationale: Browser work parallelizes well, but each persisted import replaces source flags and marks unseen records missing for that source. Sequential writes make it easier to verify candidate counts and avoid importing incomplete crawls.
    Date/Author: 2026-05-24 / Codex.
...
The full AA pass has now completed and produced a publishable snapshot. The latest checked database state contains 96,548 active AA meetings, up from the baseline of 114, with 101,570 active meetings total across AA, CA, and NA. The snapshot is not blocked by open error flags. Listed meeting contact details and online meeting credentials no longer create warnings, all open AA missing-timezone warnings have been cleared, and low scrape-confidence warnings have been reduced to 51 rows that still need source-specific extraction repair.
